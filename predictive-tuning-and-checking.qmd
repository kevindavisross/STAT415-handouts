...
(Known population SD is an unrealistic assumption that we use for simplicity here.)
    ...
    We remember that we shouldn't assign 0 prior probability to possible values, no matter how implausible, so we pick a fairly wide range of possible values for $\theta$, namely^[Uniform on [3, 13] is enough to illustrate the idea we're getting at. You could try Uniform on [0, 24] and the issue we'll see is even more extreme.] Uniform on [3, 13].
...
- Posterior predictive checks can reveal problems in proposed models. 
However, even if the checks uncover no issues, that does not confirm that the model is "correct"; it's just not obviously wrong.
...
ggplot(data.frame(y_sim), aes(x = y_sim)) +
  ...
  labs(x = "Number of successes", y = "Simulated relative frequency") +
  theme_bw()
...
ggplot(data.frame(switches_sim), aes(x = switches_sim)) +
  geom_bar(
    aes(y = after_stat(prop)),
    col = bayes_col["posterior_predict"],
    fill = bayes_col["posterior_predict"],
  ...
  labs(x = "Number of switches", y = "Simulated relative frequency") +
  theme_bw()
