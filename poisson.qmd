...
1. Suppose that we observe 4 games with a total of 12 home runs.
Find the posterior distribution of $\theta$.
In particular, how do you determine the likelihood column?
\
\
...
bayes_table |>
  ...
  pivot_longer(!theta, names_to = "source", values_to = "prob") |>
  mutate(source = fct_relevel(source, "prior", "likelihood", "posterior")) |>
  ggplot(aes(x = theta, y = prob, col = source)) +
  geom_point(size = 3) +
...
data |>
  count(hr) |>
  adorn_totals("row") |>
  kbl(col.names = c("Number of HRs", "Number of games")) |>
  kable_styling()
```
```{r}
sum(data$hr)
...
ggplot(data, aes(x = hr)) +
  ...
  scale_x_continuous(
    breaks = 0:(max(data$hr) + 1),
    limits = c(0, max(data$hr) + 1)
  ) +
  labs(x = "Number of HRs", y = "Proportion of games") +
  theme_bw()
```
...
bayes_table |>
  ...
  mutate(likelihood = likelihood / sum(likelihood)) |>
  select(theta, prior, likelihood, posterior) |>
  pivot_longer(!theta, names_to = "source", values_to = "probability") |>
  mutate(source = fct_relevel(source, "prior", "likelihood", "posterior")) |>
  ggplot(aes(x = theta, y = probability, col = source)) +
...
ggplot(data.frame(x = c(0, 6)), aes(x = x)) +
  ...
  stat_function(
    fun = dgamma,
    args = list(shape = alpha_prior, rate = lambda_prior),
    lty = bayes_lty["prior"],
    linewidth = 1,
  ...
  stat_function(
    fun = dgamma,
    args = list(shape = alpha_posterior, rate = lambda_posterior),
...
y_sim_samples = matrix(y_sim, ncol = n, nrow = trunc(length(y_sim) / n))
