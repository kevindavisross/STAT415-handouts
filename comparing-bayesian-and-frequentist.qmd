...
- All inference is based on the posterior distribution of parameters which quantifies our uncertainty in the parameters, given the observed sample data.
...
- The posterior distribution quantifies a ranking of which values are more or less plausible as a possible value of the parameter, and how much more/less plausible.
- Testing a null hypothesis versus an alternative hypotheses plays a much smaller role


In a frequentist approach

- Parameters are treated as fixed (not random), but unknown numbers
- So it doesn't make sense to make probability statements about parameters
- Data are treated as random
- All inference is based on the sampling distribution of the data which quantifies how the data would behave over many hypothetical samples.
...
- Any value inside the confidence interval is a plausible value for the parameter. 
Any value outside the confidence interval is not a plausible value of the parameter. 
A value is either plausible or not; the cut-off is arbitrary (and conventionally 95%). 
Aside from this, there is no concept of some values being more plausible than others.
- Traditional tests of null versus alternative hypotheses don't really answer interesting questions (e.g., they focus on "is there a difference?" as opposed to "is the difference large or meaningful?")
