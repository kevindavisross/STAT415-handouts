# Introduction to Inference for a Population Mean {#sec-normal-mean}
...
What does the standard deviation $\sigma=1.1$ represent?
\
\
\
\
...
1. Now assume a prior distribution which is proportional to a Normal distribution with mean 98.6 and standard deviation 0.3 over $\theta$ values in the grid.
...
1. The posterior distribution is approximately Normal with posterior mean 97.75 and posterior SD 0.074.
...
sampling_distribution_table = expand_grid(theta, y = seq(95, 102, 0.01)) |>
  mutate(prob = dnorm(y, theta, sigma / sqrt(n)))

sampling_distribution_table |>
  mutate(theta = factor(theta)) |>
  ggplot(aes(x = y, y = prob, col = theta)) +
  geom_point() +
  ...
  annotate(
    ...
    xmax = y_obs + 0.05,
    ymin = 0,
    ymax = max(sampling_distribution_table["prob"]),
    alpha = 0.5,
...
likelihood_table |>
  ...
  ggplot(aes(x = theta, y = likelihood, col = theta_f)) +
  geom_point(size = 3) +
  scale_color_viridis_d(option = "magma") +
  scale_x_continuous(breaks = theta) +
  labs(y = paste("Likelihood of y = ", y_obs, " given theta"), col = "theta") +
  theme_bw()
```
...
The likelihood is determined by evaluating the Normal($\theta$, 1.1) density at $y=97.9$ for different values of $\theta$: `dnorm(97.9, theta, 1.1)` or

$$
f(97.9|\theta)  = \frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.9-\theta}{1.1}\right)^2\right)
$$

For example, the likelihood for $\theta = 98.6$ is

$$
\frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.9-98.6}{1.1}\right)^2\right),
$$

or `dnorm(97.9, 98.6, 1.1)` = 0.296.
...
sigma = 1.1 # assumed known population SD
...
The likelihood is determined by evaluating the Normal($\theta$, 1.1) density at $y=97.5$ for different values of $\theta$: `dnorm(97.5, theta, 1.1)` or

$$
f(97.5|\theta)  = \frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.5-\theta}{1.1}\right)^2\right)
$$

For example, the likelihood for $\theta = 98.6$ is

$$
\frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.5-98.6}{1.1}\right)^2\right),
$$

or `dnorm(97.5, 98.6, 1.1)` = 0.220.
...
Since the two measurements are independent, the likelihood is the product of the likelihoods for $y=97.9$ and $y=97.5$: `dnorm(97.9, theta, 1.1) * dnorm(97.5, theta, 1.1)` or

$$
f((97.9, 97.5)|\theta)  = \left(\frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.9-\theta}{1.1}\right)^2\right)\right)\left(\frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.5-\theta}{1.1}\right)^2\right)\right)
$$

For example, the likelihood for $\theta = 98.6$ is

$$
f((97.9, 97.5)|\theta=98.6)  = \left(\frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.9-98.6}{1.1}\right)^2\right)\right)\left(\frac{1}{\sqrt{2\pi}}\,\exp\left(-\frac{1}{2}\left(\frac{97.5-98.6}{1.1}\right)^2\right)\right)
$$

or `dnorm(97.9, 98.6, 1.1) * dnorm(97.5, 98.6, 1.1)` = (0.296)(0.218) = 0.065.
...
The likelihood is determined by evaluating the Normal($\theta$, $\frac{1.1}{\sqrt{2}}$) density at $\bar{y}=97.7$ for different values of $\theta$: `dnorm(97.7, theta, 1.1 / sqrt(2))` or

$$
f_{\bar{Y}}(97.7|\theta)  \propto \exp\left(-\frac{1}{2}\left(\frac{97.7-\theta}{1.1/\sqrt{2}}\right)^2\right)
$$

For example, the likelihood for $\theta = 98.6$ is

$$
f_{\bar{Y}}((97.7)|\theta=98.6)  \propto \exp\left(-\frac{1}{2}\left(\frac{97.7-98.6}{1.1/\sqrt{2}}\right)^2\right)
$$

or `dnorm(97.7, 98.6, 1.1 / sqrt(2))` = 0.263.
...
For example, the ratio of the likelihoods for $\theta=98.6$ and $\theta=98.1$ is the same in both parts: $0.4494/0.2626 = 1.71$ and $0.1115/0.0652=1.71$.
...
# prior
prior = dnorm(theta, 98.6, 0.3) # shape of prior
prior = prior / sum(prior) # scales so that prior sums to 1
...
```{r}
# posterior probability theta is less than 98.6
