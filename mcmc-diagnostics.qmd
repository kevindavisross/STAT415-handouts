...
Tn |>
  filter(step <= 100) |>
  pivot_longer(cols = !step, names_to = "chain", values_to = "theta") |>
  ggplot(aes(x = step, y = theta, col = chain)) +
...
- **Warm up** (or "burn in") refers to the process of discarding the first several hundred or thousand steps of the chain to allow for a "settling in" period.
...
for (r in 1:Nruns) {
  for (nn in nstop) {
    plot(
      Tn[1:nn, r],
      type = "l",
      main = paste("delta=", sigma[r], "; first ", nn, " steps", sep = ""),
      xlab = "n",
      ylab = "Xn",
      ylim = c(0, 1)
    )
...
par(mfrow = c(1, Nruns), mar = c(5, 4, 0, 2) + 0.1)
for (r in 1:Nruns) {
  acf(Tn[-(1:1000), r])
  title(
    paste(
      ...
      sigma[r],
      "; ESS=",
      round(effectiveSize(Tn[-(1:1000), r]), 0),
      sep = ""
    ),
    line = -1
  )
...
for (k in 1:3) {
  ...
  hist(
    cilowers[, k],
    main = "",
    xlab = "Estimate of lower CI endpoint",
    xlim = c(0, 0.20)
  )
}
```
