...
In essence, what they really learn is that the population for island $\theta=2$ is $(2^2 e^{-0.5(2)})/(1^2 e^{-0.5(1)}) = 1.472/0.606 = 2.426$ times greater than the population for island $\theta = 1$.
...
- The Metropolis algorithm requires symmetric proposals: $j(\theta_1\to \theta_2) = j(\theta_2\to \theta_1)$. The Metropolis-Hastings algorithm generalizes the algorithm to allow asymmetric proposal distributions, but the acceptance probabilities need to be adjusted to account for any asymmetry.
...
::: {#exm-mcmc-beta}
Suppose we wish to estimate $\theta$, the proportion of Cal Poly students who have read a non-school related book in 2026.
Assume a Beta(1, 3) prior distribution for $\theta$.
In a sample of 25 Cal Poly students, 4 have read a book in 2026.
...
- We will most often use MCMC methods to simulate values from a posterior distribution $\pi(\theta|y)$ of parameters $\theta$ given data $y$. (Remember: $\theta$ is usually a vector of parameters, and $y$ represents all the data---all the observations and variables.)
...
::: {#exm-mcmc-curry}
We have seen how to estimate a process probability $p$ in a Binomial situation, but what if the number of trials is also random?  For example, suppose we want to estimate both the average number of three point shots Steph Curry attempts per game ($\mu$) and the probability of success on any single attempt ($p$).  Assume that
...
for (i in 2:n_steps) {
  ...
  if (!(proposed %in% theta)) {
    # to correct for proposing moves outside of boundaries
    proposed = current
  }
...
```{r}
pi_theta(0.15) / pi_theta(0.20)

pi_theta(0.25) / pi_theta(0.20)
```
...
theta = data.frame(mu = rep(NA, n_steps), p = rep(NA, n_steps))
...
for (n in 2:n_steps) {
  ...
  accept = min(
    1,
    pi_theta(proposed$mu, proposed$p) / pi_theta(current$mu, current$p)
  )
